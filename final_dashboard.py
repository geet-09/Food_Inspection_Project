...
boston_food_inspection_cleaned=boston_food_inspection.replace({'city':{'BOSTON':'Boston','DORCHESTER':'Dorchester', 'JAMAICA PLAIN':'Jamaica Plain', 'WEST ROXBURY':'West Roxbury','SOUTH BOSTON':'South Boston','ROSLINDALE':'Roslindale','BRIGHTON':'Brighton','EAST BOSTON':'East Boston', 'MATTAPAN':'Mattapan' }})
    ...
    html.Div(
        [
            html.H4('Number of rows to display:'),
                dcc.Slider(id="num_row_slider", min=0, max=min(25, len(df)),step=5, value=10,
                marks={i:str(i) for i in range(len(df)+5)}),
            html.H4('Enter City'),
